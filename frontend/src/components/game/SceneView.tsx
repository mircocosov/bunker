...
export function SceneView({ players, layers, groundYPercent = 74, showGroundLine = false }: Props) {
  const [failedLayerIds, setFailedLayerIds] = useState<Record<string, true>>({});
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      {sortedLayers
        .filter((layer) => !failedLayerIds[layer.id])
        .map((layer) => (
          <img
            key={layer.id}
            src={layer.assetKey}
            className="absolute inset-0 h-full w-full object-cover"
            onError={() => setFailedLayerIds((prev) => ({ ...prev, [layer.id]: true }))}
            style={{
              zIndex: layer.zIndex,
